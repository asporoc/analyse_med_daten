...
counts <- data %>%
  count(Gender, supervisor) %>%
  group_by(Gender) %>%
  mutate(percentage = n / sum(n) * 100)  # Calculate percentage within each Gender

# Plot using ggplot2
ggplot(counts, aes(x = Gender, y = percentage, fill = supervisor)) +
  geom_bar(stat = "identity", position = "stack") +
  labs(title = "Would you be willing to discuss a mental health issue with your direct supervisor(s)?",
       x = "Geschlecht",
       y = "Prozent",
       fill = "discuss with supervisor?") +
