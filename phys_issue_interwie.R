...
counts <- data %>%
  count(Gender, phys_health_interview) %>%
  group_by(Gender) %>%
  mutate(percentage = n / sum(n) * 100)  # Calculate percentage within each Gender

# Plot using ggplot2
ggplot(counts, aes(x = Gender, y = percentage, fill = phys_health_interview)) +
  geom_bar(stat = "identity", position = "stack") +
  labs(title = "Would you bring up a physical health issue with a potential employer in an interview?",
       x = "Geschlecht",
       y = "Prozent",
       fill = "Would you discuss physical issue in interview") +
